...
from backend.app.agent.api import _RateLimiter, create_agent_router
...
def test_comparison_view_carries_products_rows_and_winner():
    ...
    reply = _compare_flow(compare_state([a, b]), deps, [])

    assert reply.comparison is not None
    assert [p.productidweb for p in reply.comparison.products] == ["a", "b"]
    noise = next(r for r in reply.comparison.rows if r.label == "Độ ồn")
    # Lower dB wins for a numeric_lower dimension.
    assert noise.winner_id == "a"
    assert noise.values == {"a": "24 dB", "b": "30 dB"}
    assert reply.comparison.price_delta == 2_000_000
...
def test_comparison_row_is_dropped_when_a_side_is_a_placeholder():
    ...
    assert reply.comparison is not None
    assert all(row.label != "Độ ồn" for row in reply.comparison.rows)
...
def test_non_rankable_dimension_has_no_winner():
    ...
    row = next(r for r in reply.comparison.rows if r.label == "Loại máy")
...
def test_comparison_is_absent_without_two_products():
    ...
    reply = _compare_flow(state, deps, [])
...
def test_comparison_table_agrees_with_the_reply_text():
    ...
    noise = next(r for r in reply.comparison.rows if r.label == "Độ ồn")
    winner_name = next(
        p.name for p in reply.comparison.products if p.productidweb == noise.winner_id
    )
