...
export function parseAgentResponse(value: unknown): AgentResponse {
  ...
  // US-126 representative imagery: rendered only when the server both marks it
  // representative and supplies a URL that survives the same safety guard as
  // every other agent-supplied link.
  const imageUrl = parseSafeUrl(value.image_url);
  if (imageUrl && value.image_type === "representative") {
    response.image_url = imageUrl;
    response.image_type = "representative";
    if (typeof value.mapping_version === "number") {
      response.mapping_version = value.mapping_version;
    }
  }
  return response;
}
